...
const nativeDetectClueLy = lib.func(
  "detect_cluely",
  CluelyDetectionResultStruct,
  [],
);
...
const nativeGetClueLyWindowCount = lib.func(
  ...
  public static detectCluelyDetailed(): CluelyDetection {
    ...
    if (result.screen_capture_evasion_count > 0) {
      evasionTechniques.push(
        `Screen capture evasion (${result.screen_capture_evasion_count} windows)`,
      );
    }
    if (result.elevated_layer_count > 0) {
      evasionTechniques.push(
        `Elevated layer positioning (${result.elevated_layer_count} windows)`,
      );
    }
